...
# ╔═╡ f9ed01a1-39c3-485e-8e8e-3ca266a83651
@bind reset Button("Reset datasheet")
...
begin
	...
	exps = dataset["EXPERIMENTS"] |> 
		...
		_.Genotype, _.Age, _.Condition, 
		_.Photoreceptor, _.Channel, 
		_.RMAX_fit, _.rmax
		}) |> 
		@orderby(_[orderby[1]]) |> @thenby(_[orderby[2]]) |>
	collect
	exps = vcat(("No Experiment"), exps)
md"

### 2) Pick a experiment from the list to exclude
...
begin
	if selectExps == ["No Experiment"] || isempty(selectExps) 
		qTRACE = nothing
		qEXP = nothing
	else
		qTRACE = matchExperiment(dataset["TRACES"], selectExps[1])
		qEXP = matchExperiment(dataset["EXPERIMENTS"], selectExps[1])
		fig_summary = plot_data_summary(qTRACE, qEXP)
		fig_summary
	end
end
...
begin
	reload
	md"""
	Include experiment $(@bind INCLUDE CheckBox(default=true))
	"""
end
...
begin
	reload
	if !INCLUDE && !isnothing(qTRACE) && !isnothing(qEXP)
		println("Flagging Experiment")
...
begin
	reload
	plot_dataset_vals(dataset, metricX = :Rmax, metricXSEM = :Rmax_sem)
end

# ╔═╡ 619ed622-bdf7-4e0b-a1b1-5162a8bd4925
begin
	reload
	plot_dataset_vals(dataset, metricX = :Time_to_peak, metricXSEM = :Time_To_Peak_sem)
end

# ╔═╡ c7ee157c-34c0-4e83-997e-1e836e0038f7
begin
	reload
	plot_dataset_vals(dataset, metricX = :Percent_Recovery, metricXSEM = :Percent_Recovery_sem)
end
