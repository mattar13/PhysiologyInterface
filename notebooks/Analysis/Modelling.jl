...
# ╔═╡ ec645cd0-bb0e-11ed-230e-6fde52787f7b
begin
	using PlutoUI
	...
	using Symbolics
	#Light dependant delay in the falling phase of the retinal rod response
	#
end

# ╔═╡ 16245f7a-35b0-4d1e-a649-902f70df7fea
md"
Build the Phototransduction cascade and OS currents
"

# ╔═╡ dc885f8f-b2b5-4f38-aa9c-88cf98478565
begin
	@parameters t 
	D = Differential(t)

	@parameters cm gC gL EL gKV EK gCa ECa
	...
	@parameters F V1 S1 δ
	@variables hv(t) I_DARK(t) V(t)
	@variables mKV(t) hKV(t) mCa(t) mKCa(t)
	@variables Ca(t) CaB(t)
	@variables _Ca_s(t) _Ca_f(t)
 	@variables _CaB_ls(t) _CaB_hs(t)
	@variables _CaB_lf(t) _CaB_hf(t)
	@variables R(t) Ra(t)
	@variables Trαβγ(t) Trα(t) Trβγ(t) Trαi(t)
	@variables PDE(t) PDEa(t)
	@variables cGMP(t) GMP(t)
	@variables I_CEX(t)
end;

# ╔═╡ 896be29c-8ec1-4a3f-8d94-5d45a499908c
begin
	#Current equations
	I_photo(v, cGMP) = -J(cGMP) * (1.0 - exp((v - 8.5) / 17.0))
	I_L(v) = gL * (v - EL)
	I_KV(v) = gKV * mKV^3 * hKV * (v - EK)
	I_Ca(v, _Ca_s) = gCa * mCa^4 * hCa(v) * (v - ECa)
	I_Cl(v, _Ca_s) = gCl * mCl(_Ca_s) * (v - E_Cl)
	...
	I_ALL(v, cGMP, _Ca_s) = I_photo(v, cGMP) + I_L(v) + I_KV(v) + I_Ca(v, _Ca_s) + I_Cl(v, _Ca_s) + I_KCa(v, _Ca_s) + I_ex(v, _Ca_s) + I_ex2(_Ca_s) + I_h(v)
end

# ╔═╡ d59d2313-d486-494d-9013-46d8b0111e29
begin
	#Auxillary function J
	...
	R_Ca_(V, _Ca_s) = -(
          (I_Ca(V, _Ca_s) + I_ex(V, _Ca_s) + I_ex2(_Ca_s)) / (2 * F * V1)
     ) * 10e-6 
end

# ╔═╡ a310a24b-1ff2-4666-bb53-c1786ba74f83
u0 = [
	hv => 0.0      #Photons/μm
	I_DARK => 0.0  #Dark current
	I_CEX => 0.0
...
# ╔═╡ a6d4c3b6-00cb-493e-b1e1-b17ce7e668e1
p0 = [
	...
	V1 => 3.812e-13#Another const          DEFAULT: 3.812e-13 dm^3
	S1 => 3.142e-8 #const                   DEFAULT: 3.142e-8 dm^2
	δ => 3e-5      #Calcium area            DEFAULT: 3.0e-5

];

# ╔═╡ 61b256d1-dcd7-4d7a-a952-761554401619
begin
	rxn_net = @reaction_network begin
		($hv*αC, K1r), R <--> Ra #Isomerization of Rhodopsin
		K2f*Ra, Trαβγ --> Trα + Trβγ #Transducin activation
		K3f, PDE + Trα --> PDEa #PDE activation
		K3r, PDEa --> PDE + Trαi
		K2r, Trαi + Trβγ --> Trαβγ  
		(K4*PDEa, A(Ca)), cGMP <--> GMP #PDE mediated breakdown of cGMP
		(b*gC*J(cGMP), γCa*(1-C0/Ca)), 0 <--> Ca
		(K6f*(er - CaB), K6r), Ca <--> CaB	
		(1.0, 1.0), _Ca_s <--> _Ca_f 
	end
	@named REACTIONS = convert(ODESystem, rxn_net)

	eqns = [
		D(hv) ~ 0.0
		D(I_DARK) ~ gC*J(cGMP)*(V-8.5)
		D(I_CEX) ~ -(I_Ca(V,_Ca_s)+I_ex(V,_Ca_s)+I_ex2(_Ca_s))/(2*F*V1) * 10e-6 
		D(V) ~ -(I_DARK + I_L(V) + I_KV(V) + I_Ca(V, _Ca_s))/cm
		...
		D(mCa) ~ αmCa(V) * (1 - mCa) - βmCa(V) * mCa
		#This is the non-linear term for the Calcium activate potassium
		D(mKCa) ~ αmKCa(V) * (1 - mKCa) - βmKCa(V) * mKCa
		
	]
	@named VOLTAGES = ODESystem(eqns)

	OS = extend(VOLTAGES, REACTIONS)
end;

# ╔═╡ fc2102c0-a2cb-4b94-9a72-ae41b864b77e
tspan = (0.0, 10.0)

# ╔═╡ f3155f86-289b-47fa-a0e7-9ad5fca878df
@bind HV NumberField(1.0:1e6, default = 100.0)

# ╔═╡ b2d0ef7e-fa40-4a05-8b28-3675978b6836
HV

# ╔═╡ dc35e315-aaa7-4a4e-8928-b089f501f5cf
begin #Make the light stimulus
	stim_length = 1.0
	stim_start = 1.0
	tstops1 = [stim_start, stim_start+stim_length]
	affect1!(integrator) = integrator.u[15] = HV
	cb1 = PresetTimeCallback(tstops1, affect1!)
	
	tstops2 = [stim_start+stim_length+1e-6]
	affect2!(integrator) = integrator.u[15] = 0.0
	cb2 = PresetTimeCallback(tstops2, affect2!)
	
	cbs = CallbackSet(cb1, cb2)
end

# ╔═╡ 969ac75f-f239-4751-b551-45997505938b
begin
	prob = ODEProblem(OS, u0, tspan, p0)
	sol = solve(prob, TRBDF2(), callback = cbs, tstops = [tstops1..., tstops2...])
end;

# ╔═╡ 7586b0b8-6a2f-44d1-83a8-e019cd994302
begin
	Time = sol.t
	Vt = sol.(Time, idxs = V)
...
# ╔═╡ 320037b1-167b-4d09-8497-33f3367f0808
begin 
	fig3, ax3 = plt.subplots(4)
	ax3[1].plot(Time, hvt)
	ax3[2].plot(Time, sol.(Time, idxs = _Ca_f))
	ax3[2].plot(Time, sol.(Time, idxs = _Ca_s))
	fig3
end

# ╔═╡ 314b336c-7724-4fb3-9e5f-f2974eb02a62
begin
	fig, ax = plt.subplots(5,1)
	ax[1].plot(Time, hvt)
	#ax[1].plot(Time, It)
	ax[1].set_ylabel("Phot")
	ax[1].xaxis.set_visible(false)
	
	ax[2].plot(Time, Rt, label = "R")
	ax[2].plot(Time, Rat, label = "R*")
	ax[2].set_ylabel("Rh")
	ax[2].xaxis.set_visible(false)
	ax[2].legend()
	
	ax[3].plot(Time, Trαβγt, label = "Gnabyt")
	ax[3].plot(Time, Trαt, label = "Gnat*")
	ax[3].plot(Time, Trαit, label = "Gnati")
	ax[3].plot(Time, Trβγt, label = "Gnbyt")
	ax[3].set_ylabel("GNAT")
	ax[3].legend()
	ax[3].xaxis.set_visible(false)
	
	ax[4].plot(Time, PDEt)
	ax[4].plot(Time, PDEat)
	ax[4].set_ylabel("PDE")
	ax[4].xaxis.set_visible(false)
	
	ax[5].plot(Time, cGMPt, label = "cGMP")
	ax[5].plot(Time, GMPt, label = "GMP")
	ax[5].set_ylabel("cGMP")
	ax[5].legend()
	fig
end

# ╔═╡ 9811f7f6-e214-4753-80fd-44ab8b320c3f
begin
	fig2, ax2 = plt.subplots(4,1)
	ax2[1].plot(Time, hvt)
	#ax2[1].plot(Time, It)
	
	ax2[2].plot(Time, cGMPt)
	ax2[2].plot(Time, GMPt)
...
# ╔═╡ 131518b4-a539-470e-a7ac-a73cc77a33d0
Cat[end]

# ╔═╡ d05f8384-7255-448b-93e1-2538876b7ccb
CaBt[end]
...
git-tree-sha1 = "6a8afd2e1a3c4302bfea5bd39c1866dbfa4875e9"
